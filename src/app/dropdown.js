...
export default function ToggleContent() {
  ...
  useEffect(() => {
    if (!i18n.isInitialized) return; // Ensure translations are loaded
  
    const p = t("paths");
    console.log(`Fetching files for path: ${p} with language: ${i18n.language}`);
  
    fetchFiles(p)
      .then(setData)
      .catch((err) => console.error(`Error fetching path=${p}`, err));
  }, [i18n.language, i18n.isInitialized]);
  // useEffect(() => {
  //   const p = t("paths");
  //   fetchFiles(p)
  //     .then(setData)
  //     .catch(console.error(`parhs=${(p)}`));
  // }, []);
